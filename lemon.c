...
{
  static char templatename[] = "lempar.go";
  char buf[1000];
  FILE *in;
  char *tpltname;
  char *cp;
...
void print_stack_union(
  ...
  */
  ...
  fprintf(out,"type %sTOKENTYPE %s\n",name,
    lemp->tokentype?lemp->tokentype:"interface{}");  lineno++;
  ...
  fprintf(out, "type YYMINORTYPE interface{}\n");
  *plineno = lineno;
  /*--------------------- COPY PASTED from ^^^
  name = lemp->name ? lemp->name : "Parse";
  lineno = *plineno;
...
void ReportTable(
  ...
  if( mhflag ){
    char *name = file_makename(lemp, "_tokens.go");
  ...
  fprintf(out,"const YYERRORSYMBOL = %d\n",
	  lemp->errsym->useCnt ? lemp->errsym->index : -1); lineno++;
  tplt_xfer(lemp->name,in,out,&lineno);
  ...
  */
  ...
  fprintf(out,"var yy_action = []YYACTIONTYPE{\n"); lineno++;
  for(i=j=0; i<n; i++){
...
void ReportHeader(struct lemon *lemp)
{
  ...
  in = file_open(lemp,"_tokens.go","rb");
  if( in ){
    for(i=1; i<lemp->nterminal && fgets(line,LINESIZE,in); i++){
      sprintf(pattern,"const %s%-30s = %2d\n",prefix,lemp->symbols[i]->name,i);
  ...
  out = file_open(lemp,"_tokens.go","wb");
  if( out ){
    fprintf(out,"package main\n");
    for(i=1; i<lemp->nterminal; i++){
      fprintf(out,"const %s%-30s = %2d\n",prefix,lemp->symbols[i]->name,i);
    }
    fclose(out);  
